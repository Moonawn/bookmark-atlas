...
Collection stores media metadata, not binaries. When saved settings contain `media: images`, `sync` also downloads photos and video covers. Use `--media none` to skip this for one run. Existing settings default to no automatic downloads. `fetch-media` can also be run independently: it downloads photos, keeps a single frame for videos, and records each file's original URL and owning post. Videos are framed rather than downloaded by default — one long video can outweigh the entire rest of the archive — and `--with-videos` brings the binaries down when that is actually wanted. An oversized file keeps its thumbnail and a recorded skip reason: report it as skipped, never as complete.

Media failures do not invalidate a successful collection. Report `media.failed` separately and continue compiling available text; the next run retries incomplete or missing assets. `sync` refreshes `exports/bookmarks.json`, including current hashes for earlier sources. Avoid running a second download pass when sync already handled media.
